...
export default function TrainerDashboard() {
  ...
  }, [allSessions, filterStatus, searchQuery]);
  ...
  const isDateFullyBooked = (date: string) => {
    const activeSessions = getSessionsForDate(date).filter((session: any) => session.status !== "cancelled");
    return activeSessions.length >= 8;
  };
    ...
    <Layout>
      <header className="mb-8 pt-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-4xl md:text-5xl font-display text-white">
                TRAINER <span className="text-primary text-glow">DASHBOARD</span>
              </h1>
              <p className="text-silver mt-2 uppercase tracking-widest text-sm">
                Kevin's Training Management
              </p>
            </div>
          </div>
          <Button
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          <StatsCard title="Total Sessions" value={stats.totalSessions} icon={CalendarDays} color="text-primary" />
          <StatsCard title="Today" value={stats.todaySessions} icon={Clock} color="text-green-400" />
          <StatsCard title="This Week" value={stats.thisWeekSessions} icon={BarChart3} color="text-blue-400" />
          <StatsCard title="Booked" value={stats.bookedSessions} icon={Users} color="text-orange-400" />
          <StatsCard title="Completed" value={stats.completedSessions} icon={CheckCircle2} color="text-green-500" />
          <StatsCard title="Cancelled" value={stats.cancelledSessions} icon={XCircle} color="text-red-500" />
        </div>
      </header>
          ...
          <div className="glass-panel rounded-2xl p-6 border border-white/10">
            ...
            <div className="space-y-4">
              ...
              <Button
                ...
                className="w-full border-white/20 text-silver hover:text-white"
              >
                {showAvailability ? <EyeOff className="w-4 h-4 mr-2" /> : <Eye className="w-4 h-4 mr-2" />}
      ...
      <AnimatePresence>
        {isManualBookingOpen && (
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-bold text-white">Manual Booking</h3>
                <button
                  onClick={() => setIsManualBookingOpen(false)}
